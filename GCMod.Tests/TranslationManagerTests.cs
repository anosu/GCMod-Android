...
public sealed class TranslationManagerTests : IDisposable
{
    ...
    [Fact]
    public void UnsupportedNovelIdClearsPreviousNovelWithoutRequestingZero()
    {
        int requests = 0;
        using var client = new HttpClient(
            new Handler(
        ...
        TranslationPatch.SetupTranslation("novel", "101");
        int before = requests;
        TranslationPatch.SetupTranslation("novel", "unsupported");
        Assert.Equal(0, manager.CurrentNovelId);
