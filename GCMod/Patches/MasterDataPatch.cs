...
public static class MasterDataPatch
{
    ...
    static NativeHook<LoadCallbackFn> _loadDetour;
    ...
    public static void Install()
    {
        lock (Lifecycle)
        {
            ...
            var detour = new NativeHook<LoadCallbackFn>(
                target,
                Marshal.GetFunctionPointerForDelegate(_loadHook)
            );
            HarmonyLib.Harmony harmony = null;
            try
            {
                harmony = new HarmonyLib.Harmony("GCMod.Android.MasterData");
                harmony.PatchAll(typeof(MasterDataPatch));
                detour.Attach();
                _loadOrig = detour.Trampoline;
                _loadDetour = detour;
                _harmony = harmony;
            }
            catch
            ...
            Logger.Info($"MasterDataPatch installed: {method.Name} @ 0x{target.ToInt64():X}");
        }
    }
    ...
    static void OnDecompressed(ref string __result)
    {
        ...
        catch (Exception e)
        {
            ...
            Logger.Error($"[MasterDataPatch] JSON rewrite failed; keeping original JSON: {e}");
        }
    }
