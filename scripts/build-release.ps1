...
$modProject = Join-Path $repoRoot "GCMod/GCMod.csproj"
...
$modAssembly = Join-Path $repoRoot "GCMod/bin/$Configuration/GCMod.dll"
...
$archivePath = Join-Path $outputDirectory "GCMod-Android.zip"
...
$unexpectedFiles = @(
    Get-ChildItem -LiteralPath $outputDirectory -Force |
        Where-Object { $_.Name -notin @("GCMod-Android.zip", "GCMod-Android.zip.tmp", "SHA256SUMS.txt") }
)
...
[System.IO.File]::WriteAllText(
    $sumsPath,
    "$archiveHash  GCMod-Android.zip`n",
